...
// Use passed options (already cached)
...
// New features
$welcome_enabled = isset($n8nchwi_options) ? $n8nchwi_options['welcome_enabled'] : 'no';
$welcome_message = isset($n8nchwi_options) ? $n8nchwi_options['welcome_message'] : '';
$powered_by = isset($n8nchwi_options) ? $n8nchwi_options['powered_by'] : 'no';
$powered_text = isset($n8nchwi_options) ? $n8nchwi_options['powered_text'] : 'Powered by n8n';
$header_logo = isset($n8nchwi_options) ? $n8nchwi_options['header_logo'] : '';
?>
<div id="n8n-chat-widget-container" class="n8n-chat-widget-container n8n-chat-widget-position-<?php echo esc_attr($position); ?>" role="region" aria-label="<?php esc_attr_e('Chat widget', 'n8n-chat-widget'); ?>">
    ...
    <?php if ($welcome_enabled === 'yes' && !empty($welcome_message)) : ?>
    <!-- Welcome Message -->
    <div id="n8n-chat-widget-welcome" class="n8n-chat-widget-welcome" role="status" aria-live="polite">
        <button type="button" class="n8n-chat-widget-welcome-close" aria-label="<?php esc_attr_e('Dismiss', 'n8n-chat-widget'); ?>">&times;</button>
        <?php echo esc_html($welcome_message); ?>
    </div>
    <?php endif; ?>

    <!-- Chat Popup -->
    <div id="n8n-chat-widget-popup" class="n8n-chat-widget-popup" role="dialog" aria-labelledby="n8n-chat-widget-title" aria-modal="true" aria-hidden="true">
        <div class="n8n-chat-widget-header">
            <div id="n8n-chat-widget-title" class="n8n-chat-widget-title">
                <?php if (!empty($header_logo)) : ?>
                    <img src="<?php echo esc_url($header_logo); ?>" alt="" class="n8n-chat-widget-header-logo">
                <?php endif; ?>
        ...
        <?php if ($powered_by === 'yes' && !empty($powered_text)) : ?>
        <div class="n8n-chat-widget-footer">
        ...
        <?php else: ?>
            <?php if (!empty($svg_icon)): ?>
                <span class="n8n-chat-widget-svg-icon">
                    ...
                    if ($attachment_id) {
                        echo wp_get_attachment_image($attachment_id, array(26, 26), false, array(
                            'alt' => esc_attr__('Chat', 'n8n-chat-widget'),
                            'class' => 'svg-icon'
                        ));
                    } else {
                        // Use the helper function for proper display
                        echo wp_kses_post(n8nchwi_display_svg($svg_icon, array(26, 26), array(
                            'alt' => esc_attr__('Chat', 'n8n-chat-widget'),
                            'class' => 'svg-icon'
                        )));
                    }
